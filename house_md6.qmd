...
undervotes <- all_votes |> 
  filter(gov == 'undervote', comptroller != 'undervote', ag != 'undervote', senate != 'undervote', congress != 'undervote') |> 
  group_by(county) |> 
  summarise(gov_under = n())

# Joining those together
totals <- all_gov_votes |>  inner_join(dem_ticket) |> inner_join(gop_ticket) |> inner_join(gop_moore) |> inner_join(mg) |> inner_join(undervotes)

totals <- totals |> mutate(pct_undervotes = gov_under/all_gov*100)
```
